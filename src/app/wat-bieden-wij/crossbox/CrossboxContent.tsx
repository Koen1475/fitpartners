...
export default function CrossboxContent() {
  ...
  return (
    <main>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 0 }} className="crossbox-grid">
            <div style={{ borderRight: "1px solid #2a2720", position: "relative" }}>
              <div className="scroll-hint" style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: 56, zIndex: 2, pointerEvents: "none", background: "linear-gradient(to right, transparent, #0c0b0a)", opacity: scrollHint ? 1 : 0, transition: "opacity 0.4s ease" }}>
                <div style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", color: "#BC9C22", fontSize: 18 }} className="hint-arrow">›</div>
              </div>
              <div ref={tabsRef} className="tabs-inner" onScroll={() => setScrollHint(false)}>
                {tabs.map((tab, i) => (
                  <button key={tab.label} onClick={() => switchTab(i)}
                    style={{ width: "100%", border: "none", cursor: "pointer", padding: "20px 28px", textAlign: "left", display: "flex", alignItems: "center", gap: 18, borderLeft: `3px solid ${active === i ? "#BC9C22" : "transparent"}`, borderBottom: "1px solid #1e1d1b", transition: "border-color 0.25s, background 0.2s", background: active === i ? "rgba(183,157,62,0.06)" : "transparent" }}
                    ...
                    <span style={{ fontFamily: "var(--font-anton)", fontSize: 13, color: active === i ? "#BC9C22" : "#2e2c29", letterSpacing: "0.05em", transition: "color 0.25s", lineHeight: 1, flexShrink: 0 }}>{tab.num}</span>
            ...
            <div className="crossbox-content" style={{ padding: "40px 48px", minHeight: 320, display: "flex", flexDirection: "column", justifyContent: "center" }}>
              <div style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(8px)", transition: "opacity 0.25s ease, transform 0.25s ease" }}>
                ...
                {tabs[active].content.map((para, i) => (
                  <p key={i} style={{ color: "#b8b3aa", fontSize: 15, lineHeight: 1.9, margin: i < tabs[active].content.length - 1 ? "0 0 18px" : 0 }}>{para}</p>
                ))}
              </div>
            </div>
          </div>
        ...
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          <Reveal>
            ...
            <p style={{ color: "#b8b3aa", fontSize: 16, lineHeight: 1.8, marginBottom: 40 }}>
              Bel ons op{" "}<a href="tel:06-14634488" style={{ color: "#BC9C22", textDecoration: "none", fontWeight: 600 }}>06-14634488</a>{" "}of stuur een bericht via onderstaande knop.
            </p>
      ...
      <style>{`
        @keyframes hintBounce { 0%, 100% { transform: translateY(-50%) translateX(0); } 50% { transform: translateY(-50%) translateX(5px); } }
        .hint-arrow { animation: hintBounce 1s ease-in-out infinite; }
        .tabs-inner { display: block; width: 100%; }
        .scroll-hint { display: none; }
        @media (max-width: 720px) {
          .crossbox-grid { grid-template-columns: 1fr !important; }
          .crossbox-grid > div:first-child { border-right: none !important; border-bottom: 1px solid #2a2720; overflow: hidden; }
          .tabs-inner { display: flex !important; overflow-x: auto !important; scrollbar-width: none !important; }
