  ...
  return (
    <div style={{ borderBottom: "1px solid #2a2720" }}>
      ...
      >
        ...
        <ChevronDown size={18} color="#b79d3e" style={{ flexShrink: 0, transition: "transform 0.25s", transform: open ? "rotate(180deg)" : "rotate(0)" }} />
      </button>
      {open && (
    ...
    <main>
      ...
      <section style={{ background: "#0c0b0a", padding: "72px 24px 96px" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", flexDirection: "column", gap: 56 }}>
          {faqs.map(cat => (
            <div key={cat.cat}>
              <h2 style={{ fontFamily: "var(--font-anton)", fontSize: 20, textTransform: "uppercase", color: "#b79d3e", margin: "0 0 8px", letterSpacing: "0.08em" }}>{cat.cat}</h2>
