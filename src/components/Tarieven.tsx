  ...
  return (
    <section id="tarieven" style={{ background: "#131211", padding: "96px 24px" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <Reveal>
          <p style={{ color: "#BC9C22", fontSize: 12, letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 16 }}>Investering</p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 24 }}>
          {plans.map((p, i) => (
            <Reveal key={p.name} delay={(i + 1) as 1 | 2 | 3}>
              <div style={{
                background: p.featured ? "#BC9C22" : "#0c0b0a",
                padding: 40,
                position: "relative",
                ...
                {p.badge && (
                  <div style={{ position: "absolute", top: -1, right: 24, background: "#0c0b0a", color: "#BC9C22", fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", padding: "6px 14px", fontWeight: 700 }}>
                    {p.badge}
                  </div>
                )}
                ...
                <ul style={{ listStyle: "none", margin: "0 0 36px", padding: 0 }}>
                  {p.features.map((f) => (
                    <li key={f} style={{ color: p.featured ? "#131211" : "#b8b3aa", fontSize: 14, padding: "8px 0", borderBottom: `1px solid ${p.featured ? "rgba(12,11,10,0.15)" : "#2a2720"}`, display: "flex", gap: 10, alignItems: "center" }}>
                      <span style={{ width: 6, height: 6, borderRadius: "50%", background: p.featured ? "#0c0b0a" : "#BC9C22", flexShrink: 0, display: "inline-block" }} />
                      {f}
                    </li>
                  ))}
                </ul>
                <a href="#contact" style={{
                  ...
                  background: p.featured ? "#0c0b0a" : "#BC9C22", color: p.featured ? "#BC9C22" : "#0c0b0a",
